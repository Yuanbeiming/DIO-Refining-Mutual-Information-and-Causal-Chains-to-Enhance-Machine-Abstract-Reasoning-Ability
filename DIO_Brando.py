...
class Brando(nn.Module):
    
    def __init__(self, *args, w = 16, low_dim = 128, num_aux_candidates = 8):
        
        super(Brando, self).__init__()
        words = 9
        ...
        self.name = "Brando_depth_" + str(depth) + '_'
        ...
        self.Brando = Infinity_Transformer.Infinity_Transformer(words = words , 
                                                                dim = self.low_dim, 
                                                                depth = depth, 
                                                                heads = num_head, 
                                                                dim_head = dim_head, 
        ...
        M = torch.zeros(depth, num_head, dim_head, dim_head, requires_grad=False)
        
        B = torch.ones(depth, num_head, dim_head, 1, requires_grad=False)*1e-9
        


        self.prompt_mu = nn.Parameter(torch.zeros(1, self.low_dim))
    ...
    def reparameterize(self, mu, logvar):
        ...
        e = torch.randn_like(mu)          # ε ~ N(0,1)                  # 重参数
        return mu + e * var 
    ...
    def forward(self, x):
        ...
        x = x[None].expand(self.num_aux_candidates, b, n, s, d).reshape(-1, n, s, d)
    

        o = self.reparameterize(self.prompt_mu[None, None].expand(b*self.num_aux_candidates, 1, self.w, self.low_dim), 
                                torch.ones(b*self.num_aux_candidates, 1, self.w, self.low_dim, device = x.device)
                                )
        
        
        x_o = torch.cat([x[:,:8] + torch.randn_like(x[:,:8]), o], dim = 1)
        
        
        
        x_o = Rearrange('b n s d -> (b s) n d')(x_o)
        
        
        M = [self.M[[i]].expand(x_o.shape[0], -1,-1,-1) for i in range(self.M.shape[0])]
        B = [self.B[[i]].expand(x_o.shape[0], -1,-1,-1) for i in range(self.B.shape[0])]
        
        
        
    
        prompt,  M, B= self.Brando(x_o, M, B)
        ...
        return prompt[:, None].reshape(b, self.num_aux_candidates, s, d)
    
    
class raven_clip(nn.Module):
    def __init__(self, *args,  num_aux_candidates = 8):
        ...
        self.name = 'DIO_Brando_aux_candidates_' + str(self.num_aux_candidates)
        ...
        self.Brando = Brando(w = self.w, low_dim = self.low_dim, num_aux_candidates = self.num_aux_candidates)
        ...
        if self.pretrain:
            ...
            pretrained_params = torch.load('./model_DIO_1200000_neutral_best.pt',map_location = 'cpu')
            
            for name, param in self.named_parameters():
                if name in pretrained_params and param.data.shape == pretrained_params[name].shape:
            ...
            for name, buffer in self.named_buffers():
                if name in pretrained_params and buffer.data.shape == pretrained_params[name].shape: 
    ...
    def freeze_block(self, *args):
        

        if self.step%2 == 0 and self.num_aux_candidates > 0:
            
            self.freeze_all_except_brando()

        else:
            
            self.freeze_brando_only()           
    ...
    def forward(self, x):
        ...
        if self.num_aux_candidates > 0:
        
            extra_candidates = self.Brando(x[:,:8])
    ...
    def loss_function(self, *out, target_shape, target_line, idx):
        ...
        idx_oht = F.one_hot(idx, 8)
        
        idx_can = 1 - idx_oht
        
        
        x_neg = x[:, :8].detach()*idx_can
        ...
        if self.num_aux_candidates > 0 and self.step%2 == 0 :
        
            for i in range(8, self.num_aux_candidates + 8):
                
                loss_5 += self.dio_loss(x_neg + x[:, [i]]*idx_oht, idx)
                
            loss_5 = loss_5/self.num_aux_candidates
            
        if self.training: self.step += 1

        return  10*loss_3 + 0*loss_4 + loss_5,  right_shape, right_line, right
